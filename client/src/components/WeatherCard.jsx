...
const getWeatherIcon = (description) => {
    ...
    if (desc.includes('broken') || desc.includes('scattered')) return <WiCloudy className="text-5xl" />;
    if (desc.includes('cloud')) return <WiCloudy className="text-5xl" />;
    if (desc.includes('rain') || desc.includes('drizzle')) return <WiRain className="text-5xl" />;
    if (desc.includes('shower')) return <WiShowers className="text-5xl" />;
    if (desc.includes('snow')) return <WiSnow className="text-5xl" />;
    if (desc.includes('mist') || desc.includes('fog') || desc.includes('haze')) return <WiFog className="text-5xl" />;
...
const WeatherCard = ({cityData, onRemove}) => {
    ...
    return (
      <div className="rounded-xl shadow-lg overflow-hidden transition-transform duration-200 hover:scale-105 flex flex-col h-full">
        {/* Top Colored Section */}
        <div className={`bg-gradient-to-br ${backgroundClass} text-white p-6 relative`}>
          {/* Close Button */}
          <button 
            onClick={(e) => {
              e.preventDefault();
          ...
          {/* City Name and Time */}
          <div className="mb-6">
            <h3 className="text-2xl font-semibold">{CityName}, {sys?.country || 'LK'}</h3>
            <p className="text-sm opacity-80">{displayTime}, {displayDate}</p>
          </div>
        ...
        <div className="bg-gray-800 text-white p-6 flex-grow">
          ...
          <div className="flex items-center justify-between text-sm pt-4 border-t border-gray-700">
            <div className="flex items-center gap-2">
              ...
              />
              <span>{wind?.speed?.toFixed(1)}m/s {wind?.deg}° {getWindDirection(wind?.deg)}</span>
            </div>
