...
const fetchWeatherData = async (cityCode) => {
  ...
  try {
    ...
    console.error(
      `Error fetching weather for CityCode ${cityCode}:`,
      error.message
    );
    return null;
  }
  ...
  try {
    ...
    for (const city of cities) {
      const isCacheExpired =
        !city.LastFetched ||
        new Date() - new Date(city.LastFetched) > CACHE_DURATION_MS;

      if (isCacheExpired) {
        const fetchPromise = fetchWeatherData(city.CityCode).then((apiData) => {
          if (apiData) {
